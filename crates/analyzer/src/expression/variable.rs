...
mod tests {
    ...
    test_analysis! {
        ...
        issues = [
            ...
            IssueCode::InvalidTypeCast, // (string) $key, possibly undefined, so `mixed`
            IssueCode::InvalidTypeCast, // (string) $value, possibly undefined, so `mixed`
            IssueCode::InvalidTypeCast, // (string) $y, possibly undefined, so `mixed`
        ]
    }
